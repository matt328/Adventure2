...
void DeviceResources::CreateDeviceResources() {
   ...
   // NOTE: Enabling the debug layer after device creation will invalidate the active device.
   {
      ...
      if (SUCCEEDED(DXGIGetDebugInterface1(0, IID_PPV_ARGS(dxgiInfoQueue.GetAddressOf())))) {
         ...
         DXGI_INFO_QUEUE_MESSAGE_ID hide[] = {
             80 /* IDXGISwapChain::GetContainingOutput: The swapchain's adapter does not control the output on which the
                   swapchain's window resides. */
             ,
         };
   ...
   m_copyCommandQueue = std::make_unique<CommandQueue>(m_d3dDevice, D3D12_COMMAND_LIST_TYPE_COPY);
   ...
   ThrowIfFailed(m_d3dDevice->CreateFence(
       m_fenceValues[m_backBufferIndex], D3D12_FENCE_FLAG_NONE, IID_PPV_ARGS(m_fence.ReleaseAndGetAddressOf())));
...
void DeviceResources::CreateWindowSizeDependentResources() {
   ...
   if (m_swapChain) {
      ...
      ThrowIfFailed(m_dxgiFactory->CreateSwapChainForHwnd(
          m_commandQueue.Get(), m_window, &swapChainDesc, &fsSwapChainDesc, nullptr, swapChain.GetAddressOf()));
...
void DeviceResources::UpdateBufferResource(Microsoft::WRL::ComPtr<ID3D12GraphicsCommandList> commandList,
   ...
   (*pIntermediateResource)->SetName(L"Intermediate Buffer");
