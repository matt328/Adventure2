...
TextConsole::TextConsole() : m_textColor(1.f, 1.f, 1.f, 1.f) { Clear(); }

TextConsole::TextConsole(ID3D12Device* device, ResourceUploadBatch& upload, const RenderTargetState& rtState,
...
void TextConsole::Render(ID3D12GraphicsCommandList* commandList) {
   ...
   for (unsigned int line = 0; line < m_rows; ++line) {
      ...
      if (*m_lines[textLine]) { m_font->DrawString(m_batch.get(), m_lines[textLine], pos, color); }
...
void TextConsole::Clear() {
   if (m_buffer) { memset(m_buffer.get(), 0, sizeof(wchar_t) * (m_columns + 1) * m_rows); }
...
void TextConsole::Format(const wchar_t* strFormat, ...) {
   ...
   if (m_tempBuffer.size() < len) m_tempBuffer.resize(len);
...
void TextConsole::SetWindow(const RECT& layout) {
   ...
   float lineSpacing = m_font->GetLineSpacing();
   unsigned int rows =
       std::max<unsigned int>(1, static_cast<unsigned int>(float(layout.bottom - layout.top) / lineSpacing));

   RECT fontLayout = m_font->MeasureDrawBounds(L"X", XMFLOAT2(0, 0));
   unsigned int columns = std::max<unsigned int>(
       1, static_cast<unsigned int>(float(layout.right - layout.left) / float(fontLayout.right - fontLayout.left)));
...
void TextConsole::RestoreDevice(ID3D12Device* device, ResourceUploadBatch& upload, const RenderTargetState& rtState,
                                const wchar_t* fontName, D3D12_CPU_DESCRIPTOR_HANDLE cpuDescriptor,
                                D3D12_GPU_DESCRIPTOR_HANDLE gpuDescriptor) {
...
void TextConsole::SetViewport(const D3D12_VIEWPORT& viewPort) {
   if (m_batch) { m_batch->SetViewport(viewPort); }
}

void TextConsole::SetRotation(DXGI_MODE_ROTATION rotation) {
   if (m_batch) { m_batch->SetRotation(rotation); }
}
