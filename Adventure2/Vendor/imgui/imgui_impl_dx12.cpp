...
// in the 'example_win32_direct12/example_win32_direct12.vcxproj' project file)
...
//  g_XXXX access changed to bd->XXXX). 2021-05-19: DirectX12: Replaced direct access to ImDrawCmd::TextureId with a
//  call to ImDrawCmd::GetTexID(). (will become a requirement) 2021-02-18: DirectX12: Change blending equation to
//  preserve alpha in output buffer. 2021-01-11: DirectX12: Improve Windows 7 compatibility (for D3D12On7) by loading
//  d3d12.dll dynamically. 2020-09-16: DirectX12: Avoid rendering calls with zero-sized scissor rectangle since it
//  generates a validation layer warning. 2020-09-08: DirectX12: Clarified support for building on 32-bit systems by
//  redefining ImTextureID. 2019-10-18: DirectX12: *BREAKING CHANGE* Added extra ID3D12DescriptorHeap parameter to
//  ImGui_ImplDX12_Init() function. 2019-05-29: DirectX12: Added support for large mesh (64K+ vertices), enable
...
void ImGui_ImplDX12_RenderDrawData(ImDrawData* draw_data, ID3D12GraphicsCommandList* ctx) {
   ...
   if (fr->VertexBuffer == NULL || fr->VertexBufferSize < draw_data->TotalVtxCount) {
      ...
      if (bd->pd3dDevice->CreateCommittedResource(&props,
...
static void ImGui_ImplDX12_CreateFontsTexture() {
   ...
   // Upload texture to graphics system
   {
      ...
      bd->pd3dDevice->CreateCommittedResource(
          &props, D3D12_HEAP_FLAG_NONE, &desc, D3D12_RESOURCE_STATE_COPY_DEST, NULL, IID_PPV_ARGS(&pTexture));
   ...
   // - Important: to compile on 32-bit systems, this backend requires code to be compiled with '#define ImTextureID
   // ImU64'.
   // - This is because we need ImTextureID to carry a 64-bit value and by default ImTextureID is defined as void*.
   // [Solution 1] IDE/msbuild: in "Properties/C++/Preprocessor Definitions" add 'ImTextureID=ImU64' (this is what we do
   // in the 'example_win32_direct12/example_win32_direct12.vcxproj' project file) [Solution 2] IDE/msbuild: in
   // "Properties/C++/Preprocessor Definitions" add 'IMGUI_USER_CONFIG="my_imgui_config.h"' and inside
   // 'my_imgui_config.h' add '#define ImTextureID ImU64' and as many other options as you like. [Solution 3]
   // IDE/msbuild: edit imconfig.h and add '#define ImTextureID ImU64' (prefer solution 2 to create your own config
   // file!) [Solution 4] command-line: add '/D ImTextureID=ImU64' to your cl.exe command-line (this is what we do in
   // the example_win32_direct12/build_win32.bat file)
...
bool ImGui_ImplDX12_CreateDeviceObjects() {
   ...
   // Create the root signature
   {
      ...
      if (d3d12_dll == NULL) {
         ...
         for (int i = 0; i < IM_ARRAYSIZE(localD3d12Paths); i++)
            if ((d3d12_dll = ::LoadLibraryA(localD3d12Paths[i])) != NULL) break;

         // If failed, we are on Windows >= 10.
         if (d3d12_dll == NULL) d3d12_dll = ::LoadLibraryA("d3d12.dll");

         if (d3d12_dll == NULL) return false;
      }
      ...
      if (D3D12SerializeRootSignatureFn == NULL) return false;

      ID3DBlob* blob = NULL;
      if (D3D12SerializeRootSignatureFn(&desc, D3D_ROOT_SIGNATURE_VERSION_1, &blob, NULL) != S_OK) return false;

      bd->pd3dDevice->CreateRootSignature(
          0, blob->GetBufferPointer(), blob->GetBufferSize(), IID_PPV_ARGS(&bd->pRootSignature));
      blob->Release();
   }
   ...
   // Create the vertex shader
   {
      ...
      if (FAILED(D3DCompile(
              vertexShader, strlen(vertexShader), NULL, NULL, NULL, "main", "vs_5_0", 0, 0, &vertexShaderBlob, NULL)))
   ...
   // Create the pixel shader
   {
      ...
      if (FAILED(D3DCompile(
              pixelShader, strlen(pixelShader), NULL, NULL, NULL, "main", "ps_5_0", 0, 0, &pixelShaderBlob, NULL))) {
