...
bool ImGui::Combo(
    const char* label, int* current_item, const char* const items[], int items_count, int height_in_items) {
...
bool ImGui::DataTypeApplyOpFromText(
    const char* buf, const char* initial_value_buf, ImGuiDataType data_type, void* p_data, const char* format) {
   ...
   if (is_logarithmic) {
      ...
      else if ((v_min * v_max) < 0.0f) // Range crosses zero, so split into two portions
      {
         float zero_point_center =
             (-(float)v_min) /
             ((float)v_max -
              (float)v_min); // The zero point in parametric space.  There's an argument we should take the logarithmic
...
static bool InputTextFilterCharacter(unsigned int* p_char,
   ...
   if (apply_named_filters && (flags & (ImGuiInputTextFlags_CharsDecimal | ImGuiInputTextFlags_CharsHexadecimal |
      ...
      // "C" locale where the decimal point is '.'. We don't really intend to provide widespread support for it, but out
      // of empathy for people stuck with using odd API, we support the bare minimum aka overriding the decimal point.
...
bool ImGui::InputTextEx(const char* label,
   ...
   if (g.ActiveId != id && init_make_active) {
      ...
      if (is_multiline) g.ActiveIdUsingKeyInputMask |= ((ImU64)1 << ImGuiKey_PageUp) | ((ImU64)1 << ImGuiKey_PageDown);
   ...
   if (g.ActiveId == id) {
      ...
      if (select_all) {
         ...
         if ((multiclick_count % 2) == 0) {
            ...
            if (!STB_TEXT_HAS_SELECTION(&state->Stb)) ImStb::stb_textedit_prep_selection_at_cursor(&state->Stb);
   ...
   if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id) {
      ...
      const bool is_validate_nav =
          (IsNavInputTest(ImGuiNavInput_Activate, ImGuiInputReadMode_Pressed) && !IsKeyPressedMap(ImGuiKey_Space)) ||
          IsNavInputTest(ImGuiNavInput_Input, ImGuiInputReadMode_Pressed);
      const bool is_cancel =
          IsKeyPressedMap(ImGuiKey_Escape) || IsNavInputTest(ImGuiNavInput_Cancel, ImGuiInputReadMode_Pressed);
      ...
      } else if (IsKeyPressedMap(ImGuiKey_Delete) && !is_readonly && !is_cut) {
   ...
   if (g.ActiveId == id) {
      ...
      // didn't do that, code like InputInt() with ImGuiInputTextFlags_EnterReturnsTrue would fail. This also allows the
      // user to use InputText() with ImGuiInputTextFlags_EnterReturnsTrue without maintaining any user-side storage
      // (please note that if you use this property along ImGuiInputTextFlags_CallbackResize you can end up with your
      // temporary string object unnecessarily allocating once a frame, either store your string data, either if you
      // don't then don't use ImGuiInputTextFlags_CallbackResize).
...
static void ColorEditRestoreHS(const float* col, float* H, float* S, float* V) {
   ...
   ImGuiContext& g = *GImGui;
   if (g.ColorEditLastColor != ImGui::ColorConvertFloat4ToU32(ImVec4(col[0], col[1], col[2], 0))) return;

   // When S == 0, H is undefined.
   // When H == 1 it wraps around to 0.
   if (*S == 0.0f || (*H == 0.0f && g.ColorEditLastHue == 1)) *H = g.ColorEditLastHue;
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col) {
   ...
   if (flags & ImGuiColorEditFlags_PickerHueWheel) {
      ...
      if (IsItemActive()) {
         ...
         if (g.ColorEditLastColor == ColorConvertFloat4ToU32(ImVec4(col[0], col[1], col[2], 0))) H = g.ColorEditLastHue;
         value_changed = value_changed_sv = true;
      }
      if (!(flags & ImGuiColorEditFlags_NoOptions)) OpenPopupOnItemClick("context");
...
bool ImGui::CollapsingHeader(const char* label, bool* p_visible, ImGuiTreeNodeFlags flags) {
   ...
   if (p_visible) flags |= ImGuiTreeNodeFlags_AllowItemOverlap | ImGuiTreeNodeFlags_ClipLabelForTrailingButton;
...
bool ImGui::ListBox(const char* label,
   ...
   clipper.Begin(items_count, GetTextLineHeightWithSpacing()); // We know exactly our line height here so we pass it as
...
int ImGui::PlotEx(ImGuiPlotType plot_type,
   ...
   if (values_count >= values_count_min) {
      ...
      for (int n = 0; n < res_w; n++) {
         ...
         // NB: Draw calls are merged together by the DrawList system. Still, we should render our batch are lower level
         // to save a bit of CPU.
...
void ImGui::EndMenuBar() {
   ...
   if (NavMoveRequestButNoResultYet() && (g.NavMoveDir == ImGuiDir_Left || g.NavMoveDir == ImGuiDir_Right) &&
      ...
      if (nav_earliest_child->ParentWindow == window &&
         ...
         // This involve a one-frame delay which isn't very problematic in this situation. We could remove it by scoring
         // in advance for multiple window (probably not worth bothering)
...
bool ImGui::BeginMenuEx(const char* label, const char* icon, bool enabled) {
   ...
   if (window->Flags & ImGuiWindowFlags_ChildMenu) flags |= ImGuiWindowFlags_ChildWindow;
...
static void ImGui::TabBarLayout(ImGuiTabBar* tab_bar) {
   ...
   if (tab_bar->ReorderRequestTabId != 0) {
      if (TabBarProcessReorder(tab_bar))
         if (tab_bar->ReorderRequestTabId == tab_bar->SelectedTabId) scroll_to_tab_id = tab_bar->ReorderRequestTabId;
   ...
   for (int tab_n = 0; tab_n < tab_bar->Tabs.Size; tab_n++) {
      ...
      // tab bar. Additionally, when using TabBarAddTab() to manipulate tab bar order we occasionally insert new tabs
      // that don't have a width yet, and we cannot wait for the next BeginTabItem() call. We cannot compute this width
      // within TabBarAddTab() because font size depends on the active window.
   ...
   if ((tab_bar->WidthAllTabsIdeal > tab_bar->BarRect.GetWidth() && tab_bar->Tabs.Size > 1) &&
      ...
      if (ImGuiTabItem* scroll_and_select_tab = TabBarScrollingButtons(tab_bar)) {
         ...
         if ((scroll_and_select_tab->Flags & ImGuiTabItemFlags_Button) == 0) tab_bar->SelectedTabId = scroll_to_tab_id;
      }
   ...
   for (int section_n = 0; section_n < 3; section_n++) {
      ImGuiTabBarSection* section = &sections[section_n];
      if (section_n == 2) tab_offset = ImMin(ImMax(0.0f, tab_bar->BarRect.GetWidth() - section->Width), tab_offset);
...
void ImGui::TabBarQueueReorderFromMousePos(ImGuiTabBar* tab_bar, const ImGuiTabItem* src_tab, ImVec2 mouse_pos) {
   ...
   for (int i = src_idx; i >= 0 && i < tab_bar->Tabs.Size; i += dir) {
      ...
      if ((dst_tab->Flags & ImGuiTabItemFlags_SectionMask_) != (src_tab->Flags & ImGuiTabItemFlags_SectionMask_)) break;
      dst_idx = i;
...
bool ImGui::TabBarProcessReorder(ImGuiTabBar* tab_bar) {
   ...
   if ((tab1->Flags & ImGuiTabItemFlags_SectionMask_) != (tab2->Flags & ImGuiTabItemFlags_SectionMask_)) return false;
