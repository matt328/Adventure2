...
// These functions and defines are for Luau.
   ...
   inline void lua_pushcfunction_x(lua_State* L, lua_CFunction fn) { lua_pushcfunction(L, fn, ""); }

   inline void lua_pushcclosure_x(lua_State* L, lua_CFunction fn, int n) { lua_pushcclosure(L, fn, "", n); }
...
#define LUA_OPLE 3
   ...
   inline int get_length(lua_State* L, int idx) { return static_cast<int>(lua_objlen(L, idx)); }
...
#define LUABRIDGE_LUA_OK LUA_OK
   ...
   constexpr bool is_integral_representable_by(T value) {
      constexpr bool same_signedness =
          (std::is_unsigned_v<T> && std::is_unsigned_v<U>) || (!std::is_unsigned_v<T> && !std::is_unsigned_v<U>);

      if constexpr (sizeof(T) == sizeof(U)) {
         if constexpr (same_signedness) return true;

         if constexpr (std::is_unsigned_v<T>) return value <= static_cast<T>(std::numeric_limits<U>::max());
   ...
   constexpr bool is_floating_point_representable_by(T value) {
      if constexpr (sizeof(T) == sizeof(U)) return true;
