...
namespace luabridge {
   namespace detail {
       ...
       *
       * Retrieves functions from metatables and properties from propget tables. Looks through the class hierarchy if
      ...
      inline int newindex_metamethod(lua_State* L, bool pushSelf) {
         ...
         for (;;) {
            ...
            if (lua_iscfunction(L, -1)) // Stack: mt, setter
            {
               ...
               if (pushSelf) lua_pushvalue(L, 1); // Stack: setter, table | userdata
       ...
       */
      inline int newindex_object_metamethod(lua_State* L) { return newindex_metamethod(L, true); }
       ...
       * Retrieves properties from propset tables.
       */
      inline int newindex_static_metamethod(lua_State* L) { return newindex_metamethod(L, false); }
       ...
       *
       * This is used for global variables or class static data members. The pointer to the data is in the first
       * upvalue.
       */
      template <class T>
      struct property_getter<T, void> {
         static int call(lua_State* L) {
            ...
            std::error_code ec;
            if (!Stack<T&>::push(L, *ptr, ec)) luaL_error(L, "%s", ec.message().c_str());
...
#endif
               std::error_code ec;
               if (!Stack<T&>::push(L, c->**mp, ec)) luaL_error(L, "%s", ec.message().c_str());

#if LUABRIDGE_HAS_EXCEPTIONS
            } catch (const std::exception& e) { luaL_error(L, "%s", e.what()); } catch (...) {
...
struct property_setter<std::reference_wrapper<T>, void>
{
    static int call(lua_State* L)
    {
       ...
       *
       * The member function pointer is in the first upvalue. The object userdata ('this') value is at top ot the Lua
       * stack.
       */
      template <class T>
       ...
       *
       * The proxy function pointer (lightuserdata) is in the first upvalue. The class userdata object is at the top of
       * the Lua stack.
       */
      template <class F>
       ...
       *
       * The proxy std::function (lightuserdata) is in the first upvalue. The class userdata object is at the top of the
       * Lua stack.
       */
      template <class F>
